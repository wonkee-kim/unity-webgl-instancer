    ...
    #define VERTEX_ANIMATION_OUTPUT(positionOS, normalOS, vertexID) VertexAnimation(positionOS, normalOS, vertexID);

    void VertexAnimation(out float3 positionOS, out float3 normalOS, uint vertexID)
    {
    ...
    // For ShaderGraph
    void VertexAnimation_float(uint vertexID, out float3 positionOS, out float3 normalOS)
    {
        VertexAnimation(positionOS, normalOS, vertexID);
    }
    void VertexAnimation_half(uint vertexID, out half3 positionOS, out half3 normalOS)
    {
        VertexAnimation(positionOS, normalOS, vertexID);
    }
