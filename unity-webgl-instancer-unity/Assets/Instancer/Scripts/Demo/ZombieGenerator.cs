...
public class ZombieGenerator : MonoBehaviour
{
    ...
    //private ArrayList _zombiesPool = new ArrayList(MAX_ZOMBIE_COUNT);
    ...
    void PrintAllScenes()
    {
        for (int i = 0; i < SceneManager.sceneCount; i++)
        {
            ...
            PrintSceneObjects(scene.GetRootGameObjects(), 1);
        }
    }

    void PrintSceneObjects(GameObject[] objects, int depth)
    {
        string indentation = new string('_', depth * 4);

        foreach (GameObject obj in objects)
        {
            Debug.Log(indentation + "* " + obj.name);
            PrintSceneObjects(GetChildObjects(obj), depth + 1);
        }
    }

    GameObject[] GetChildObjects(GameObject obj)
    {
    ...
    void PrintDontDestroyOnLoadObjects()
    {
        ...
        Debug.Log("DontDestroyOnLoad Objects:");
        PrintSceneObjects(dontDestroyOnLoadScene.GetRootGameObjects(), 1);
    }

    private void Start()
    {
        PrintAllScenes();
        PrintDontDestroyOnLoadObjects();
    }
    /*
